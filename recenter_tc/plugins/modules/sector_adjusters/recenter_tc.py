...
def run_archer(xarray_obj, varname):
    ...
    if varname in [
        ...
        LOG.warning(
            "Unsupported sensor %s / channel %s type for ARCHER, returning without recentering",
    ...
    if num_masked > 0:
        LOG.warning(
            "There are %s masked values in array of size %s, not attempting to run ARCHER",
            num_masked,
    ...
    for xobj in xobjs:
        ...
        if include_vars and curr_recenter_variables:
            curr_recentered_area_defs, curr_out_fnames = recenter_tc_area_def(
                area_def, xobj, variables=curr_recenter_variables
            )
...
def recenter_with_archer(sect_xarray, variables, area_def_to_recenter):
    ...
    if area_def_to_recenter.sector_info["vmax"] < ARCHER_REQUIRED_VMAX_KTS:
        LOG.info(
            "***SKIPPING not attempting to run archer, vmax of %s less than required %s kts",
    ...
    for varname in sorted(variables):
        ...
        in_dict, out_dict, score_dict, curr_out_fnames = run_archer(
...
def recenter_with_akima(sect_xarray, area_def):
    ...
    for curr_area_def in area_defs:
        ...
        for skey, val in sector_info.items():
            if isinstance(val, str) and skey in check_sector_info:
                LOG.info(
                    f"Missing {skey} sector information for {sector_info['synoptic_time']}"
                )
    ...
    LOG.info(
        f"***Interpolating new center from {len(clats)} best track positions, closest position {closest_idx}..."
    )
...
def recenter_tc_area_def(area_def, sect_xarray, variables):
    ...
    if is_sector_type(area_def, "tc"):
        ...
        archer_recentered_area_defs, out_fnames = recenter_with_archer(
            sect_xarray, variables, recentered_area_defs["akima"]
        )
